...
const FallReviewOption = ({ label, value, isSelected = false, onSelect }) => {
  if (!value && typeof onSelect === "function") {
    return (
      <button
        ...
        onClick={onSelect}
        className={cn(
          "flex min-h-14 w-full items-center justify-between rounded-[14px] border px-4 py-3.5 text-left transition-all",
  ...
  return (
    <RadioGroupItem
      value={value}
      className={cn(
        "group flex min-h-14 w-full items-center justify-between rounded-[14px] border px-4 py-3.5 text-left transition-all",
        "focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:[outline-color:var(--rm-fall-focus-outline)] active:scale-[0.994]",
        "[background:var(--rm-fall-review-option-bg)] [border-color:var(--rm-fall-review-option-border)] hover:[background:var(--rm-fall-review-option-bg-hover)]",
        "data-[state=checked]:[background:var(--rm-fall-review-option-selected-bg)] data-[state=checked]:[border-color:var(--rm-fall-review-option-selected-border)] data-[state=checked]:[box-shadow:var(--rm-fall-review-option-selected-shadow)]"
      )}
    >
